...
[最新验证记录（2026-09-12）](VALIDATION.md)：个人仓库当前 develop **168/168** 测试通过，报告有效，本次 Codecov 上传已确认服务端处理和归属；此前失败的历史 SHA 也已 **137/137** 通过。组织仓库的 29/29 结果属于 9 月 11 日历史验证，组织上传仍需配置 `CODECOV_TOKEN_ENDSTONE` 后验证。

最新个人报告的 gcovr XML 行覆盖率为 **81.05%**，Codecov 为 **55.61%**，两者统计口径不同。服务端全部文件与行已确认属于 XML 的 `src/` 集合；当前结果及历史指标差异说明见验证记录。
